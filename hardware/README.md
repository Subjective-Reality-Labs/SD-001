...
$I_{peak} = I_{out}$ + $\frac{(V_{in}-v_{out})\times{D})}{2\times{L}\times{freq}}$ where $D$ is duty cycle: $D = \frac{V_{out}}{V_{in}}$
...
You have to chose the proper footprint for these kind of resistors so the are rated according to anticipated power dissipation that can be calculated with $P = I^2\times{R}$. For example 33mOhm sense resistor with maximum current flowing though it at 2.5A will have to dissipate 0.21W. 0603 resistors have power rating of 1/8W and 0805 have 1/4W rating. So we need to use 0805 as it's the smallest one that fits the calculated rating.
